...
//                    }
    ...
    public void deleteChosenFilesFromCloudStorage(){
        listOfLocalElements.getSelectionModel().clearSelection();
        for (int i = 0; i < getPathsOfSelectedFilesInCloudStorage().size(); i++) {
            String absolutePath = getPathsOfSelectedFilesInCloudStorage().get(i).toString();
            Path path = Paths.get(absolutePath);
            File file = new File(getPathsOfSelectedFilesInCloudStorage().get(i).toString());
            try {
                if (file.isDirectory()) {
                    deleteContentsOfFolderRecursively(file);
                } else {
